...
state_defaults = {"chat": [], "rag": None, "highlights": [], "file_path": None, "current_page": 0, "last_debug": None}
...
with col1:
    if st.session_state.file_path:
        st.markdown(f"""
            <div style="display: flex; justify-content: space-between; gap: 12px; margin-bottom: 25px;">
                ...
                <div style="background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 15px; flex: 1;">
                    <span style="font-size: 0.65rem; color: #8b949e; text-transform: uppercase;">History</span><br>
    ...
    with chat_box:
        if not st.session_state.file_path: st.info("👋 Welcome. Please upload a document to begin.")
...
with col2:
    if st.session_state.file_path:
        ...
        display_target = "highlighted.pdf" if os.path.exists("highlighted.pdf") else st.session_state.file_path
        
        with open(display_target, "rb") as f:
            base64_pdf = base64.b64encode(f.read()).decode('utf-8')

        # Construct the Base64 URL with internal PDF navigation parameters
        # #page helps with auto-scrolling to the result
        pdf_url = f"data:application/pdf;base64,{base64_pdf}#page={st.session_state.current_page + 1}"
        
        # We use a sandbox and specific allow-scripts permission to stop Chrome from blocking the data URI
        st.markdown(f"""
