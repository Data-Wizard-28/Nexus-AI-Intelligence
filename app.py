...
# 2. CSS — only layout/custom classes, NO color overrides (theme handles those)
st.markdown("""
    <style>
    /* --- LOGO --- */
    ...
    .section-label {
        ...
        margin-bottom: 15px !important; padding-top: 5px;
    }
...
state_keys = {
    "chat": [], "rag": None, "highlights": [],
    "file_path": None, "current_page": 0, "last_debug": None,
    "total_pages": 0,
}
...
def render_pdf_page(pdf_path: str, page_num: int, dpi: int = 150):
    """Rasterise one PDF page → PNG bytes. Works on local + Streamlit Cloud."""
    try:
        doc = fitz.open(pdf_path)
...
with st.sidebar:
        ...
        if st.session_state.file_path:
            ...
            if uploaded:
                ...
                with st.spinner("🧠 Indexing..."):
                    ...
                    st.session_state.total_pages = get_pdf_page_count(path)
                    st.rerun()
    ...
    with st.container(border=True):
        ...
        selected_theme = st.selectbox(
            "Highlight Theme", list(color_map.keys()),
            label_visibility="collapsed", key="highlight_theme",
        )
        ...
        for m in st.session_state.chat:
            ...
            with st.chat_message(m["role"], avatar=avatar):
                ...
                if m["role"] == "assistant" and "meta" in m:
                    st.markdown(f"""
                        <div class="meta-label-container">
...
with col2:
    if st.session_state.file_path:
        ...
        # Refresh page count whenever the source file changes
        total = get_pdf_page_count(dp)
        if total == 0:
            st.warning("Could not open PDF.")
        else:
            # Clamp current page to valid range
            st.session_state.current_page = max(0, min(st.session_state.current_page, total - 1))
            ...
            with nav2:
                st.markdown(
                    f'<div class="nav-center">Page {st.session_state.current_page + 1} of {total}</div>',
                    unsafe_allow_html=True,
                )
            with nav3:
            ...
            # Render page as image — the only approach that works on HTTPS/deployed
            png_bytes = render_pdf_page(dp, st.session_state.current_page, dpi=150)
            if png_bytes:
                st.markdown('<div class="pdf-wrapper">', unsafe_allow_html=True)
